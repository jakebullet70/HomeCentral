...
'Repackages the database to free unused space. This is mainly relevant after large deletes.
Public Sub Vacuum 
	#if B4i
	Dim no As NativeObject = sql1
	no.RunMethod("ExecNonQueryInsideTransaction::", Array("VACUUM", Null))
	#else

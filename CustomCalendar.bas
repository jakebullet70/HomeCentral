...
Sub Class_Globals
	...
	Private lblDayTitle(8) As Label     'Day of Week (begin Sunday)
	Private lbtitle As Label         'Date	
...
Public Sub Initialize( Ww As Int,Hh As Int, BeginDate As Long, txtSize As Int)
	...
	pnlbackGround.Color = Colors.Transparent
	pnl.addview(pnlbackGround,tx,ty,(mx*7)+8dip, (my*9)+8dip)
	pnl.Visible=False
	
	lbtitle.Initialize("lbtitle")
	...
	lbtitle.Color = Colors.Transparent
	lbtitle.TextColor = g.GetColorTheme(g.ehome_clrTheme,"themeColorText")
	...
	For i=0 To 6 'Days name buttons
		...
		lblDayTitle(i).Gravity = Bit.Or(Gravity.CENTER_VERTICAL, Gravity.CENTER_HORIZONTAL)
		lblDayTitle(i).Color = Colors.Transparent
		...
		pnlbackGround.AddView( lblDayTitle(i), (i*mx)+((i+1)*1dip),  (1*my)+1dip, mx+2dip, my)
		'pnlbackGround.AddView( lblDayTitle(i), (i*mx)+((i+1)*1dip),  0dip, mx+2dip, my)
	
		'lblDayTitle(i).Text = dayName(i)
		g.setText(dayName(i),lblDayTitle(i))
		'Log(lblDayTitle(i).TextSize)
	Next
	...
	For j=0 To 5 'Days buttons		
		For i=0 To 6	
			...
			btDays(z).Color = Colors.Transparent
			'btDays(z).TextColor = c.CLR_TXT_NORMAL
			'btDays(z).Typeface = Typeface.DEFAULT
			btDays(z).tag = 0	
			'btDays(z).TextSize = RelativTextSize 
			'btDays(z).TextSize = 4 ' (lblDayTitle(1).TextSize / 2)
			btDays(z).Gravity = Bit.Or(Gravity.CENTER_VERTICAL, Gravity.CENTER_HORIZONTAL)
...
'	t8.Initialize
	...
	g.setText(NmFullMonth(CalMonth-1)  & "  " & CalDay& "  " & CalYear	,lbtitle)
	'lbtitle.Typeface = Typeface.
	'lbtitle.Text = NmFullMonth(CalMonth-1)  & "  " & CalDay& "  " & CalYear	
...
'	Next
	...
	For i=0 To 41 
		btDays(i).TextColor = g.GetColorTheme(g.ehome_clrTheme,"highlightText")
	...
	For i=(nb) To (nb-1+nbj)
		...
		If (cmm = CalMonth) And (caa = CalYear) Then
			If (z = cdd) Then
				...
				btDays(i).TextColor = g.GetColorTheme(g.ehome_clrTheme,"themeColorText")
				btDays(i).Color = g.GetColorTheme(g.ehome_clrTheme,"highlightText")
	...
	For i=0 To (nb-1) 'Prev month
		btDays(i).TextColor=Colors.LightGray
		btDays(i).Typeface = Typeface.DEFAULT_BOLD
		btDays(i).Text=m3+i	
...
Public Sub LengthMonth(yearNum As Int,monthNum As Int) As Int
	Return DateTime.GetDayOfMonth( dt.GetLastDayOfMonth(DateTime.DateParse(monthNum & "/1/" & yearNum)) )
End Sub
...
''''	lbtitle.TextColor=Colors.White	
''''		
''''	For i=0 To 6 'Days name buttons
